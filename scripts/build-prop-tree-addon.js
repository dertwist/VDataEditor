...
function usageAndExit() {
  console.log('Usage: node scripts/build-prop-tree-addon.js --arch=<arch>')
  console.log('Example: node scripts/build-prop-tree-addon.js --arch=x64')
  process.exit(1)
}
...
function main() {
  ...
  const addonDir = path.join(__dirname, '..', 'native', 'prop-tree-addon')
  const releaseDir = path.join(addonDir, 'build', 'Release')
  const builtNodePath = path.join(releaseDir, 'proptreeaddon.node')
  const archTaggedNodePath = path.join(releaseDir, `proptreeaddon-${arch}.node`)
